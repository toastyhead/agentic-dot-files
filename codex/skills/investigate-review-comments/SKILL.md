...
description: >-
  ...
  verifying every claim in the browser before assigning a real-issue or
...
2. Read the surrounding code, related callers, and tests until the behavior is
   clear.
3. Before assigning a verdict, use the browser to verify the exact claim. This
...
## Browser investigation defaults
...
Default frontend URL: `http://localhost:3001`. Use `http://localhost:3000` only
when the user explicitly says so or local context proves that is the active app.

Default test login when needed: load `E2E_EMAIL` and `E2E_PASSWORD` from the
gitignored `/Users/rizwan_respan/work/e2e/.env`. Never print, copy, or commit
the plaintext credentials.

In Codex, use the available browser path in this order:

1. If the in-app Browser plugin is available, follow `browser:control-in-app-browser`.
2. If the user's existing Chrome session is required, follow `chrome:control-chrome`.
3. If browser tools are not loaded, use `tool_search` to find the Browser or
   Chrome control tools.
4. If plugin browser control is unavailable but Playwright is installed, use
   `node_repl` or the repo's test tooling for a focused local reproduction.
...
- **Code review agent:** read-only review of the branch diff, focused on
  ...
  regressions.
- **Browser verification agent:** browser reproduction of the original issue
  against `http://localhost:3001`, using the test login above and the exact
