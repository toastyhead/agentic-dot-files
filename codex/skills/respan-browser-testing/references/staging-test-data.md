...
Create a fresh staging API key through the local app's `/platform/api-keys?action=create` flow for every run. The secret is shown once. Keep it only in ephemeral runtime state and revoke it through the Browser UI after all test scenarios, including failed runs. The UI labels deletion as `Revoke key` and performs a soft revoke.
...
```bash
RESPAN_API_KEY=<fresh-staging-key> \
...
5. Send the smallest batch that covers the scenario, using the staging wrapper and fresh key.
