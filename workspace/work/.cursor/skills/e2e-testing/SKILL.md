---
name: e2e-testing
description: Create and maintain user-flow-oriented Playwright end-to-end tests under /Users/rizwan_respan/work/e2e for the respan-frontend app, organized as one folder per page/feature. Use when the user asks to add, write, update, run, or maintain e2e/UI tests for a feature or page, or mentions Playwright, end-to-end, browser automation, or smoke tests.
---
...
2. One folder per page/feature under `e2e/tests/<feature>/`.
3. Do NOT write trivial or ad-hoc tests (for example, "is this button present", "does the header say X"). Removing a button should not require a test.
4. Keep every test isolated, deterministic, and self-cleaning.
5. Never hardcode credentials or secrets in specs; read them from `e2e/.env`.
...
- App routes live under `/platform/<page>` (for example `/platform/home`, `/platform/dashboard`, `/platform/prompts`, `/platform/datasets`). The login screen is `/login`.
- Test account: read `E2E_EMAIL` and `E2E_PASSWORD` from the gitignored
  `e2e/.env`; never print or copy the plaintext credentials into durable files.
...
1. Create `package.json`, `playwright.config.ts`, `tsconfig.json`, `.gitignore`, `.env.example`, `.env`, and `tests/auth.setup.ts`.
...
3. Add only the 1-3 most important secondary flows and failure states whose breakage would actually hurt the user (validation errors, empty states, permission gating, destructive confirmations).
4. Skip everything cosmetic or trivial.

Write a test only if a break would do one of these:

- Block the user's primary goal, or
- Cause data loss or show wrong data, or
...
1. Confirm the route and the primary user goal. If unclear, ask the user, or explore `respan-frontend/src/pages/PlatformPages/<Page>` to understand the flow.
2. Create `e2e/tests/<feature>/<feature>.spec.ts`.
3. Rely on the shared `storageState` for auth (do not log in inside the spec).
4. Build the flow with role/text/placeholder locators (see conventions below).
5. Make any created data unique (timestamp or uuid) and clean it up in `afterEach`/`afterAll`.
6. Run the spec locally, remove flakiness, and confirm it passes 3 runs in a row.

## Locator conventions (match respan-frontend)

- Prefer text/placeholder/label locators: `getByText`, `getByPlaceholder`, `getByLabel`, and `getByRole` for standard roles.
- IMPORTANT — buttons: this app's `ButtonNew` component exposes its variant class (for example `button-md-default`) as the button's accessible name, so `getByRole('button', { name })` does NOT match the visible label. Match buttons by text with the shared helper `getButtonByText(page, 'Add credits')` from `tests/helpers/locators.ts` (mirrors `respan-frontend/tests/helpers/locators.ts`).
...
- No hard waits (`page.waitForTimeout`). Wait on conditions with `expect(...)` / `locator.waitFor()`.
- One user journey per `test`; group related journeys with `test.describe`.
- Each test is independent and order-agnostic; never share mutable state between tests.
...
- For any screenshot assertion, mask dynamic content (timestamps, names) and allow a small pixel tolerance.
...
- Hardcoded credentials or secrets in specs.
- `waitForTimeout`, order-dependent tests, and shared mutable state.
- Asserting on implementation details (class names, internal component state).
...
- If a test is flaky, fix the root cause (better locator or wait). Do not paper over it with sleeps.
