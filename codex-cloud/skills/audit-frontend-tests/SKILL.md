...
- Keep generated audit output outside the target repository unless the user asks for an artifact. Do not stage, commit, publish, or comment on a PR unless explicitly asked.
...
Resolve the current checkout with `git rev-parse --show-toplevel`, then run the
scanner from the installed cloud skill path:

```bash
python3 "$HOME/.agents/skills/audit-frontend-tests/scripts/scan_frontend_tests.py" \
