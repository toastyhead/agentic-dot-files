...
def build_pr_command(
    ...
    *extra_arguments: str,
    ...
    command = ["gh", "pr", action]
    if args.pr_number is not None:
        command.append(str(args.pr_number))
    if args.github_repo:
...
def parse_args() -> argparse.Namespace:
    ...
    if args.pr_number is not None and not args.github_repo:
        parser.error("--pr-number requires --github-repo")
    return args
...
def main() -> int:
    ...
    pr = json.loads(
        run(
            build_pr_command(
                "view", args, "--json", "number,url,body,headRefOid"
            ),
            repo_root,
        )
    )
    ...
    verified = json.loads(
        run(
            build_pr_command("view", args, "--json", "body,headRefOid,url"),
            repo_root,
        )
    )
